...
function onFix(p){
  ...
  if(pts.length){ const d=kmDist(pts[pts.length-1],o); if(d<2) km+=d; }
  ...
  if(follow) map.setView([o.lat,o.lon], Math.max(map.getZoom(),15));
...
async function ort(p){ if(!p) return 'Ingen'; const k=key(p); if(geoCache.has(k)) return geoCache.get(k);
  try{
    const url='https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat='+p.lat+'&lon='+p.lon+'&zoom=14&addressdetails=1';
    const r=await fetch(url,{headers:{'Accept':'application/json'}});
    const d=await r.json(); const a=d.address||{};
    const name=a.town||a.city||a.village||a.municipality||a.county||'Okänd';
    geoCache.set(k,name); return name;
  }catch(e){ return 'Okänd'; }
}

// Export aktuell resa (sammanfattning + samplingsrad)
$('exportNowBtn').onclick=async()=>{
  if(!start){ alert('Ingen resa startad.'); return; }
  const t={ id:start.toISOString(), date:start.toISOString().slice(0,10), startTime:start.toISOString(), stopTime:stop?stop.toISOString():null, totalKm:Number(km.toFixed(3)), points:pts.slice() };
  ...
  const rows=[["Tid","Ort"]]; const step=Math.max(1, Math.floor(Math.max(1,t.points.length)/20));
  for(let i=0;i<t.points.length;i+=step){ rows.push([new Date(t.points[i].ts).toISOString(), await ort(t.points[i])]); }
...
// IndexedDB – rensa/period (samma som tidigare fullversioner)
...
$('exportRangeBtn').onclick=async()=>{
  const list=await allTrips(); const fd=$('fromDate').value||'0000-01-01'; const td=$('toDate').value||'9999-12-31';
  const sel=list.filter(t=> (t.date||'')>=fd && (t.date||'')<=td ); if(!sel.length){ alert('Inga resor'); return; }
  const wb=XLSX.utils.book_new(); const rows=[["Datum","Start","Stop","Km","Startort","Slutort"]];
  ...
  const f=fd.replace(/[^0-9A-Za-z_-]/g,'-'); const tt=td.replace(/[^0-9A-Za-z_-]/g,'-');
  XLSX.writeFile(wb,'period_'+f+'_till_'+tt+'.xlsx');
};
