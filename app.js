...
function init(){map=L.map('map');L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19}).addTo(map);polyline=L.polyline([],{color:'#1976d2'}).addTo(map);map.setView([58.5877,16.1924],12);}init();
function dist(a,b){const R=6371;const dLat=(b.lat-a.lat)*Math.PI/180,dLon=(b.lon-a.lon)*Math.PI/180;const h=Math.sin(dLat/2)**2+Math.cos(a.lat*Math.PI/180)*Math.cos(b.lat*Math.PI/180)*Math.sin(dLon/2)**2;return R*2*Math.atan2(Math.sqrt(h),Math.sqrt(1-h));}
function update(){kmEl.textContent=`Totalt: ${totalKm.toFixed(2)} km`}
$('startBtn').onclick=()=>{points=[];totalKm=0;startTime=new Date();statusEl.textContent='Registrerar…';polyline.setLatLngs([]);if(marker){map.removeLayer(marker);marker=null;}watchId=navigator.geolocation.watchPosition(onPos,()=>{}, {enableHighAccuracy:true});$('startBtn').disabled=true;$('stopBtn').disabled=false;}
$('stopBtn').onclick=()=>{if(watchId){navigator.geolocation.clearWatch(watchId);watchId=null;}stopTime=new Date();statusEl.textContent='Klar';$('startBtn').disabled=false;$('stopBtn').disabled=true;$('exportExcelBtn').disabled=$('exportPdfBtn').disabled=$('mailBtn').disabled=points.length===0;saveTripLocal(build())}
...
$('mailBtn').onclick=()=>alert('Mailfunktionen kräver server-backend och fungerar ej på GitHub Pages.')
$('historyBtn').onclick=async()=>{const list=await listTrips();const box=$('historyList');box.innerHTML='';list.forEach(t=>{const d=document.createElement('div');d.textContent=t.date+' '+t.totalKm+' km';d.style.padding='8px';d.style.border='1px solid #ccc';d.style.margin='6px 0';d.onclick=()=>{points=t.points;totalKm=t.totalKm;polyline.setLatLngs(t.points.map(p=>[p.lat,p.lon]));update();};box.appendChild(d);});$('historyModal').classList.remove('hidden');document.getElementById('map').classList.add('map-disabled');}
$('closeHistory').onclick=()=>{$('historyModal').classList.add('hidden');document.getElementById('map').classList.remove('map-disabled');}
})();
