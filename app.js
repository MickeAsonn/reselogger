...
async function clearDB(){const db=await openDB();return new Promise((res,rej)=>{const tx=db.transaction(STORE,'write');tx.objectStore(STORE).clear();tx.oncomplete=res;tx.onerror=()=>rej(tx.error);});}

$('cleanEmptyBtn').onclick=async()=>{const list=await allTrips();const empt=list.filter(t=>!t.points||!t.points.length);if(!empt.length)return alert('Inga tomma');if(!confirm('Rensa '+empt.length+' tomma?'))requestAnimationFrame(()=>{});for(const t of empt)await delTrip(t.id);alert('Klart');};
