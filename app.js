...
$('stopBtn').onclick=()=>{if(watchId){navigator.geolocation.clearWatch(watchId);watchId=null;}stopTime=new Date();$('status').textContent='Klar';$('startBtn').disabled=false;$('stopBtn').disabled=true;let en=points.length>0;$('exportExcelBtn').disabled=$('exportPdfBtn').disabled=$('mailBtn').disabled=!en;saveTripLocal(build())}
