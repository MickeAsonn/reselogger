// ALWAYS redirect PWA → Safari for working GPS
if (window.navigator.standalone === true) {
    // Open same URL in Safari (same window)
    window.location.href = window.location.href.replace(/#$/, '');
}

let map, marker, polyline, watchId=null, pts=[], km=0, start=null, stop=null, follow=true;
const $=id=>document.getElementById(id);
function dist(a,b){const R=6371;const dLat=(b.lat-a.lat)*Math.PI/180;const dLon=(b.lon-a.lon)*Math.PI/180;const la1=a.lat*Math.PI/180,la2=b.lat*Math.PI/180;const h=Math.sin(dLat/2)**2+Math.cos(la1)*Math.cos(la2)*Math.sin(dLon/2)**2;return R*2*Math.atan2(Math.sqrt(h),Math.sqrt(1-h));}
...
function onFix(p){
  ...
  if(pts.length){ const d=dist(pts[pts.length-1],o); if(d<2) km+=d; }
  else { map.setView([o.lat,o.lon],15); }
...
$('cleanEmptyBtn').onclick=async()=>{ const list=await allTrips(); const empties=list.filter(t=> !t.points || !t.points.length); if(!empties.length) return alert('Inga tomma resor'); if(!confirm('Rensa '+empties.length+' tomma resor?')) return; const db=await openDB(); const tx=db.transaction(STORE,'readwrite'); for(const t of empties){ tx.objectStore(STORE).delete(t.id); } tx.oncomplete=()=>alert('Klart'); tx.onerror=()=>alert('Fel vid rensning'); };
$('clearAllBtn').onclick=async()=>{ if(!confirm('Rensa ALL historik?')) return; await clearAll(); alert('Historik rensad'); };

// Reverse geocoding (Ort) – sampling, with cache
const geoCache=new Map();
function cacheKey(p){ return (Math.round(p.lat*1000)/1000)+','+(Math.round(p.lon*1000)/1000); }
async function ortFor(p){
  if(!p) return 'Ingen position';
  const k=cacheKey(p); if(geoCache.has(k)) return geoCache.get(k);
  const qs=new URLSearchParams({format:'jsonv2',lat:String(p.lat),lon:String(p.lon),addressdetails:'1',zoom:'14','accept-language':'sv',email:'reselogger@example.com'});
  try{ const r=await fetch('https://nominatim.openstreetmap.org/reverse?'+qs.toString(),{headers:{'Accept':'application/json'}}); if(!r.ok) throw new Error('HTTP '+r.status); const d=await r.json(); const a=d.address||{}; let o=a.town||a.city||a.village||a.municipality||a.hamlet||a.suburb||a.neighbourhood||a.county||''; if(!o){ const dn=(d.display_name||'').split(','); if(dn.length) o=dn[0].trim(); } if(!o) o='Okänd ort'; geoCache.set(k,o); return o; } catch(e){ return 'Okänd ort'; }
}
...
$('exportRangeBtn').onclick=async()=>{
  ...
  const safeFrom=from.replace(/[^0-9A-Za-z_-]/g,'-'); const safeTo=to.replace(/[^0-9A-Za-z_-]/g,'-');
