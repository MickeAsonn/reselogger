// === Standalone (hemskärm) fallback: visa knapp som öppnar Safari via window.open(...,"_top") ===
(function(){
  if (window.navigator.standalone === true) {
    ...
    document.body.innerHTML = `
      <div class="fallbackBox">
        ...
        <button id="openSafariBtn">Öppna i Safari</button>
      </div>`;
    // Vänta in DOM och bind klick (user gesture)
    setTimeout(()=>{
      const btn = document.getElementById('openSafariBtn');
      if(btn){ btn.addEventListener('click', ()=> { window.open(target, '_top'); }); }
    }, 50);
    // Kör INTE resten av appen i standalone
    window.__standaloneBlock = true;
  }
})();

if(!window.__standaloneBlock){
// ===== Ren v9-funktionalitet nedan =====
...
// Export aktuell resa (med orter + samplade punkter)
