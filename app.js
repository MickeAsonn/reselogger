...
$('stopBtn').onclick=()=>{
  ...
  $('exportExcelBtn').disabled = $('exportPdfBtn').disabled = $('mailBtn').disabled = !ok;
  saveTripLocal(build());
};
...
function onPos(pos){
  ...
  if(points.length){
    const km = dist(points[points.length-1], p);
    if(km < 2){ totalKm+=km; update(); }
...
$('clearLastBtn').onclick=async()=>{
  const t=await listTrips();
  if(!t.length) return alert('Ingen historik');
  if(!confirm('Rensa senaste?')) return;
  await delTrip(t[t.length-1].id);
  alert('Rensad');
};
...
function rows(t){
  const r=[["Datum",t.date],["Start",t.startTime],["Stop",t.stopTime],["Km",t.totalKm],[],["Tid","Lat","Lon"]];
  for(const p of t.points){ r.push([new Date(p.ts).toISOString(),p.lat,p.lon]); }
  return r;
}
...
$('exportExcelBtn').onclick=()=>{
  ...
  const ws=XLSX.utils.aoa_to_sheet(rows(t));
  XLSX.utils.book_append_sheet(wb,ws,'Resa');
  XLSX.writeFile(wb,`resa_${t.date}.xlsx`);
};

$('exportPdfBtn').onclick=()=>{
  const t=build(); const{jsPDF}=window.jspdf; const doc=new jsPDF(); let y=10;
  rows(t).forEach(r=>{ doc.text(String(r),10,y); y+=10; if(y>280) doc.addPage(); });
  doc.save(`resa_${t.date}.pdf`);
};

$('mailBtn').onclick=()=> alert('Ingen backend på GitHub Pages — mail fungerar ej.');

// SAFARI-PROOF MODAL FIX — KILL MAP COMPLETELY
$('historyBtn').onclick=async()=>{
  const list=await listTrips(); const box=$('historyList'); box.innerHTML='';
  list.forEach(t=>{
    const d=document.createElement('div');
    d.textContent=t.date+' '+t.totalKm+' km';
    d.style.padding='8px'; d.style.margin='6px 0'; d.style.border='1px solid #ccc';
    d.onclick=()=>{
      points=t.points;
      totalKm=t.totalKm;
