...
$('stopBtn').onclick=async()=>{
  ...
  // Spara resan i IndexedDB (FIX) så periodexport hittar den
...
// Lokal datumsträng (sv-SE) som nyckel för dagliga filter, undviker UTC-förskjutning
...
async function buildTrip(){
  const t={
    ...
    date: start? dateKeyLocal(start): dateKeyLocal(new Date()),
...
$('exportRangeBtn').onclick=async()=>{
  ...
  // Sortera på datumnyckeln (yyyy-mm-dd, lokalt) – inkluderar gränser
  ...
  const f=from.replace(/[^0-9A-Za-z_-]/g,'-'), tt=to.replace(/[^0-9A-Za-z_-]/g,'-'); XLSX.writeFile(wb,'resor_'+f+'_till_'+tt+'.xlsx');
};
