...
$('cleanEmptyBtn').onclick=async()=>{const list=await allTrips();const empt=list.filter(t=>!t.points||!t.points.length);if(!empt.length)return alert('Inga tomma');if(!confirm('Rensa '+empt.length+' tomma?'))return;for(const t of empt)await delTrip(t.id);alert('Klart');};
